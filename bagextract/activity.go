...
func (a *Activity) Execute(ctx context.Context, params *Params) (*Result, error) {
	if _, err := os.Stat(filepath.Join(params.Path, "bagit.txt")); err != nil {
		...
		return &Result{Path: params.Path}, nil
	}
	...
	// Delete everything except the data directory and files in the Keep list.
	for _, e := range entries {
		entryName := e.Name()
		if entryName == "data" || slices.Contains(params.Keep, entryName) {
			continue
		}
